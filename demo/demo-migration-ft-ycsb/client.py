"""YCSB migration + fault tolerance client.

Usage:
    python client.py <threads> <start_n_partitions> <end_n_partitions>
                     <messages_per_second> <seconds> <save_dir>
                     <warmup_seconds> <n_entities>
                     <migration_at_sec> <kill_at_sec>

- migration_at_sec: second at which to trigger migration (-1 = disabled)
- kill_at_sec:      second at which to `docker kill styx-worker-1` (-1 = disabled)
"""
...
N_ENTITIES = int(sys.argv[8])
MIGRATION_AT_SEC = int(sys.argv[9])
KILL_AT_SEC = int(sys.argv[10])
...
def ycsb_init(styx: SyncStyxClient, operator: Operator, num_workers: int = None):
    ...
    if os.path.exists(ycsb_dataset_path):
        ...
        with Pool(processes=num_workers or cpu_count()) as pool:
            results = []
            for result in tqdm(pool.imap_unordered(generate_partition_batch_wrapper, tasks), total=len(tasks)):
...
def benchmark_runner(proc_num) -> dict[bytes, dict]:
    ...
    for cur_sec in range(seconds):
        # Kill a worker at the specified second (only proc 0)
        if proc_num == 0 and 0 <= KILL_AT_SEC == cur_sec:
            subprocess.run(["docker", "kill", "styx-worker-1"], check=False)
            print(f"KILL -> styx-worker-1 at second {cur_sec}")
        ...
        for i in range(messages_per_second):
            ...
            future = styx.send_event(operator=operator, key=key, function=func_name)
        ...
        if lps < 1:
            time.sleep(1 - lps)
        sec_end2 = timer()
        print(f"{cur_sec} | Latency per second: {sec_end2 - sec_start}")

        # Trigger migration at the specified second (only proc 0)
        if cur_sec == MIGRATION_AT_SEC and proc_num == 0:
            ...
            print(f"Migration request submitted at second {cur_sec}")
...
def main():
    ...
    print(f"Migration at second: {MIGRATION_AT_SEC}, Kill at second: {KILL_AT_SEC}")
...
if __name__ == "__main__":
    ...
    print()
    calculate_metrics.main(messages_per_second, threads, warmup_seconds, SAVE_DIR)
