...
class Sequencer:
    ...
    def set_sequencer_id(self, peer_ids: list[int], worker_id: int) -> None:
        ...
        for w_id in sorted_w_ids:
            ...
            s_id_assignment = w_id if w_id in available_s_ids else min(available_s_ids)
    ...
    def set_wal_values_after_recovery(
        self,
    ...
    def increment_epoch(
        ...
        if t_ids_to_reschedule:
            ...
            self.distributed_log = (
                sorted(
                    [item for item in self.current_epoch if item.t_id in t_ids_to_reschedule],
                )
                + self.distributed_log
            )
    ...
    def get_aborted_sequence(
        self,
        t_ids_to_reschedule: set[int],
    ) -> list[SequencedItem]:
        if t_ids_to_reschedule:
            aborted_sequence: list[SequencedItem] = sorted(
                {item for item in self.current_epoch if item.t_id in t_ids_to_reschedule},
            )
